...
class Storage {
  ...
  removeItem(value) {
    if (!this.items.includes(value)) {
      console.log(`Error, not found item ${value}`);
    }
    this.items.splice(this.items.indexOf(value), this.items.indexOf(value));
  }
}
...
class Car {
  ...
  static checkPrice(price) {
    if (price > this.#MAX_PRICE) {
      return console.log('Error! Price exceeds the maximum');
    }
    return console.log('Success! Price is within acceptable limits');
  }
  constructor({ price }) {
    this.price = price;
  }
